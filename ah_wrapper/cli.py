...
def search_by_price_cmd(args: argparse.Namespace) -> None:
    try:
        ...
        print(
            f"Found {len(products)} products for '{args.query}' around €{args.price}:"
        )
        print("-" * 50)
...
def main() -> None:
    ...
    search_parser.add_argument(
        "--sort",
        choices=["relevance", "price-low", "price-high", "name-a-z", "name-z-a"],
        help="Sort order",
    )
    ...
    price_parser.add_argument(
        "--tolerance", type=float, default=0.10, help="Price tolerance (default: 0.10)"
    )
